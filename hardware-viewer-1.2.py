...
class Linux硬件驱动查看器:
    ...
    def 初始化关于页面(self):
        ...
        关于内容 = [
            ...
            "仓库地址：https://gitee.com/ocean123455/hardware-viewer/tree/master/ \n"
            f"{'='*60}\n"
        ]
        ...
        self.标签页["关于"].tag_add("title", "2.0", "3.0")  # 软件名称行
        self.标签页["关于"].tag_add("title", "4.0", "6.0")  # 软件描述标题
        self.标签页["关于"].tag_add("title", "9.0", "11.0") # 功能特点标题
        self.标签页["关于"].tag_add("title", "16.0", "18.0")# 开源致谢标题
        self.标签页["关于"].tag_add("title", "23.0", "24.0")# 开源协议标题
        self.标签页["关于"].tag_add("title", "26.0", "27.0")# 使用提示标题
    ...
    def 获取CPU信息(self):
        ...
        # 字段映射表，使用正则表达式精确匹配字段名
        字段映射 = {
            r'^model name\s*:': "CPU型号",
            r'^cpu cores\s*:': "物理核心数",
            r'^siblings\s*:': "逻辑核心数",
            r'^vendor_id\s*:': "厂商ID",
            r'^cpu MHz\s*:': "主频（MHz）",
        ...
        for 行 in cpu原始信息.split("\n"):
            行 = 行.strip()
            # 使用正则匹配字段，忽略前后空格
            for 英文模式, 中文字段 in 字段映射.items():
                if re.match(英文模式, 行, re.IGNORECASE):
                    # 提取字段值（冒号后面的内容）
                    字段值 = 行.split(":", 1)[1].strip() if ":" in 行 else 行
    ...
    def 刷新所有信息(self):
        """刷新所有硬件信息（多线程优化版）"""
        # 禁用刷新按钮，防止重复点击
        self.刷新按钮.config(state=tk.DISABLED, text="🔄 刷新中...")
        self.主窗口.update_idletasks()
    
        # 更新状态标签为"刷新中"
        self.状态标签.config(text="上次更新：刷新中...")
    
        # 定义后台刷新函数
        def 后台刷新():
            try:
                # 预计算所有硬件信息（耗时操作在后台线程执行）
                新信息 = {
                    "系统概览": self.获取系统概览(),
                    "CPU信息": self.获取CPU信息(),
                    "内存信息": self.获取内存信息(),
                    "磁盘信息": self.获取磁盘信息(),
                    "网卡信息": self.获取网卡信息(),
                    "PCI设备(驱动)": self.获取PCI设备信息(),
                    "USB设备(驱动)": self.获取USB设备信息(),
                    "显卡信息": self.获取显卡信息()
                }
            
                # 计算完成后，通过主线程更新UI（Tkinter要求UI操作在主线程）
                def 更新UI():
                    ...
                    # 更新内容
                    for 标签页名称, 内容 in 新信息.items():
                        self.标签页[标签页名称].delete(1.0, tk.END)
                        self.标签页[标签页名称].insert(tk.END, 内容)
                    ...
                    self.刷新按钮.config(state=tk.NORMAL, text="🔄 刷新硬件信息")
            
                # 让主线程执行UI更新
                self.主窗口.after(0, 更新UI)
                ...
                def 显示错误():
                    ...
                    self.刷新按钮.config(state=tk.NORMAL, text="🔄 刷新硬件信息")
                    self.状态标签.config(text=f"上次更新：刷新失败")
            
                self.主窗口.after(0, 显示错误)
    
        # 启动后台线程执行刷新
        刷新线程 = threading.Thread(target=后台刷新)
        刷新线程.daemon = True  # 线程随主程序退出
        刷新线程.start()
